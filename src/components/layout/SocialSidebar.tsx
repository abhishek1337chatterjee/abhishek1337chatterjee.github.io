...
const XIcon: FC<{ size?: number; className?: string; style?: React.CSSProperties }> = ({
  size = 18,
  className = '',
  style = {},
}) => (
...
function SocialIcon({ social }: { social: SocialLink }) {
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`px-3 py-1.5 rounded-lg bg-gradient-to-r ${social.gradient} text-white text-sm font-medium whitespace-nowrap shadow-lg`}
        >
          {social.name}
        </div>
        {/* Arrow */}
      ...
      <motion.a
        ...
        style={{
          boxShadow: isHovered
            ? `0 0 20px ${social.hoverColor}40, 0 0 40px ${social.hoverColor}20`
            : 'none',
        }}
      >
  ...
  return (
    <div className="fixed bottom-6 right-6 z-40 lg:hidden">
      ...
      >
        ...
        >
          {/* Horizontal line */}
          <motion.span className="absolute top-1/2 left-0 w-full h-0.5 bg-white rounded-full -translate-y-1/2" />
          {/* Vertical line */}
